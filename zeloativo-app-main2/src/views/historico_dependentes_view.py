...
def historico_dependentes_view(page: ft.Page, patient_id: str | None = None, on_unlinked=None):
    page.bgcolor = "#FFFFFF"
    ...
    def on_unlink():
        try:
            resp = DataService.unlink_patient(patient_id)

            if isinstance(resp, dict) and resp.get("detail"):
                show_error(resp["detail"])
                return

            page.snack_bar = ft.SnackBar(ft.Text("Vínculo removido!"), open=True)
            page.update()

            # ✅ avisa o main.py pra recarregar a aba 3
            if callable(on_unlinked):
                on_unlinked()
        ...
        return ft.Column(
            ...
            controls=[
                ft.ElevatedButton(
                    content=ft.Row(
                        [
                            ft.Icon(ft.Icons.LINK_OFF, color=ft.Colors.WHITE),
                            ft.Text("DESVINCULAR", color=ft.Colors.WHITE, weight=ft.FontWeight.BOLD),
                        ],
                    ...
                    on_click=lambda e: on_unlink(),
                ),

                ft.Text(
                    "Adesão do dependente nos últimos 7 dias",
                    size=18,
                    weight=ft.FontWeight.BOLD,
